...
(function (root, factory) {
  ...
  const PATTERN_RULES = [
    ...
    // 에러성 키워드로 끝나는 문장 → 같은 줄에 행동 지침("하세요" 등)이 없을 때
    {
      ...
      pattern: /(?:오류|실패|불가능|처리할 수 없|등록할 수 없|확인할 수 없|만료|초과|완료되지 않|처리되지 않|되지 않았)[^.。\n]*?(?:습니다|됩니다|입니다)[.。](?!(?:[^.。\n]{0,120})(?:하세요|해 주세요|해주세요|바랍니다|하시기 바랍니다|눌러|선택하|입력하))/g,
      message: () => '에러 메시지에 행동 지침("~하세요")이 없습니다. 상황+원인+행동 3단 구조로 완성하세요.',
      alt: '"파일 크기가 초과되었습니다. 5MB 이하로 줄여 다시 업로드해 주세요."',
    },

    // 슬래시 병렬 나열: "신청/이수", "신고/납부" 등 (GOV.UK: 슬래시 대신 "또는"/"과" 권고)
    {
      id: 'slash-parallel',
      name: '슬래시 병렬 나열',
      severity: 'warning',
      pattern: /[가-힣]{1,8}\/[가-힣]{1,8}/g,
      message: (match) => `슬래시 병렬 "${match}" — "또는"/"과"/"및"으로 풀어쓰세요.`,
      alt: '"신청 또는 이수", "신고 및 납부" 등 명확한 접속사 사용',
    },

    // 불명확 링크 텍스트: "바로가기>", "여기를 클릭" (GOV.UK·WCAG 2.4.6)
    {
      id: 'ambiguous-link-text',
      name: '불명확 링크 텍스트',
      severity: 'warning',
      pattern: /바로가기\s*[>\)»]|(?:여기|클릭)\s*(?:를|을)\s*(?:클릭|눌러)/g,
      message: (match) => `불명확 링크 텍스트 "${match}" — 링크 대상을 구체적으로 명시하세요.`,
      alt: '"민원 신청하기", "서류 다운로드" 등 목적지를 설명하는 텍스트 사용',
    },

    // 안내·에러 메시지에 느낌표: 공공 서비스에 부적절 (국어원·GOV.UK)
    {
      id: 'exclamation-in-guidance',
      name: '안내 메시지에 느낌표',
      severity: 'warning',
      pattern: /(?:입력하세요|확인하세요|선택하세요|시도하세요|신청하세요|이용하세요)[!！]/g,
      message: (match) => `"${match}" — 공공 서비스 안내에 느낌표는 부적절합니다.`,
      alt: '느낌표 제거하고 마침표 사용',
    },
  ...
  function isSimpleReplacement(alt) {
    ...
    // ","도 이 사전에서 거의 전부 "고객님, 신청인"처럼 복수 대안을 나열하는
    // 용도로만 쓰인다(18개 항목 감사로 확인, 2026-08-20 codex 리뷰) — 콤마가
    // 있으면 텍스트에 그대로 넣었을 때 "고객님, 신청인"처럼 어색한 나열이 그대로
    // 삽입되어 완성된 대체 문구가 아니게 된다.
