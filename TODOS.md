...
### TODO-024: AI 지적 가치 검증 실험 설계 — ✅ 1차 검증 완료 (2026-08-20)
**Priority**: P1
...
**1차 검증 결과** (`research/ai-vs-rules-validation-2026-08-20.md`): 실제 API/인프라 없이, `krds-lint.js` 실제 규칙 엔진으로 8개 샘플을 채점하고 principles.md 자신의 기준(3단 구조, 결론우선)으로 사람이 대조 판정. **8개 중 4개(입력값 오류/지원 제외 통보/모호한 재확인 요청/창의적 이중부정)가 규칙 엔진에서 100점·이슈 0건을 받았지만 principles.md가 이미 "문제"로 명시한 유형** — 특히 복지 급여 거부 통보("지원 대상에서 제외되었습니다")처럼 심리적 충격이 큰 문장이 완전히 통과됨을 확인. 최초 설계 전제("맥락 판단은 규칙만으론 안 된다")가 근거 있는 관찰임이 확인됨(가설 검증 완료, People 필요한 정량 실사이트 표본화는 별도 후속 과제로 남김). Next Steps #1 API 계약은 최소한 (1)3단 구조 완결성 (2)이중부정 의미 모호성 (3)정보 배치 순서 3가지를 판단할 수 있어야 함을 구체적으로 확인.

### TODO-025: ET3 데이터 거버넌스 설계 — raw 문장 export의 개인정보/기밀 리스크 — ✅ 초안 완료 (2026-08-20, 사용자 최종 승인 대기)
**Priority**: P1
